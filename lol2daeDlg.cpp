...
UINT Clol2daeDlg::convertWorkerThread(LPVOID lParam)
{
	...
	ColladaWriter::Mode mode;
	mode = (dlg->m_IncludeSkeleton.GetCheck() == 1) ? ColladaWriter::Mode::Skeleton : ColladaWriter::Mode::Mesh;
	...
	try
	{
		...
		if (mode >= ColladaWriter::Mode::Skeleton)
		{
			...
			mode = (anmPath.Compare(L"") != 0) ? ColladaWriter::Mode::Animation : ColladaWriter::Mode::Skeleton;
